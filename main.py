# import sys
# sys.path.append("/home/admin001/YY/Scholarship/CVPR2023-CMPAE-main")
...
from mutual_loss import Uncertainty_Guided_Mutual_Alignment_Loss
...
def train(args, model, train_loader, optimizer, criterion, epoch):
    print("----------------------------------------------------------")
    model.train()
    criterion2 = LabelSmoothingNCELoss(classes=25, smoothing=args.nce_smooth)
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        criterion_mutual = Uncertainty_Guided_Mutual_Alignment_Loss(eta=args.mutual_eta)
...
def main():
    ...
    model = MMIL_Net(args.num_layers, args.temperature, args.att_dropout, args.cls_dropout, args.nhead).to('cuda')
    ...
    if args.mode == 'train':
        ...
        model.load_state_dict(resume['model'])
        thres_candi = [0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7]
        ...
        for target_class in range( args.start_class, args.start_class + (7 if args.gpu==3 else 6)):
            best_select_metric = 0
            best_v_thres = 0
            for i, v_thres in enumerate(thres_candi):
                audio_seg, visual_seg, av_seg, avg_type_seg, avg_event_seg, \
                audio_eve, visual_eve, av_eve, avg_type_eve, avg_event_eve, avg_all \
                    = eval(args, model, test_loader, args.label_test, v_thres=v_thres, target_class=target_class)
