...
def print_overall_metric(F_seg_a, F_seg_v, F_seg, F_seg_av, F_event_a, F_event_v, F_event, F_event_av):
    ...
    # 计算并打印所有指标的平均值
    all_metrics = [audio_segment_level, visual_segment_level, av_segment_level, avg_type_seg, avg_event_seg,
                audio_event_level, visual_event_level, av_event_level, avg_type_eve, avg_event_eve]
    average_all = sum(all_metrics) / len(all_metrics)
    ...
    print('Audio Event Detection Segment-level F1, Visual Event Detection Segment-level F1, '
          ...
          'Event-level Type@Avg. F1, Event-level Event@Avg. F1,  average_all. F1')
    print('{:.1f}, {:.1f}, {:.1f}, {:.1f}, {:.1f}, {:.1f}, {:.1f}, {:.1f}, {:.1f}, {:.1f}, {:.1f}'.
          format(audio_segment_level, visual_segment_level, av_segment_level, avg_type_seg, avg_event_seg,
                 audio_event_level, visual_event_level, av_event_level, avg_type_eve, avg_event_eve, average_all))
